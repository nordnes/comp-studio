...
<template>
  <div class="bg-surface-white rounded border border-outline-gray-1 p-5">
    <div class="flex items-center gap-2 mb-3"><span class="lucide-trending-down size-3.5 text-ink-gray-5" aria-hidden="true" /><div class="text-sm text-ink-gray-5">Equity dilution · base path</div></div>
    <div class="flex items-end gap-1 h-20">
      <div v-for="(s, i) in w.steps" :key="s.id" class="flex-1 flex flex-col items-center justify-end">
        <div class="text-xs tabular-nums" :class="i === w.steps.length - 1 ? 'text-ink-amber-3' : 'text-ink-gray-5'">{{ fPct(pctOfGrant(s.N), 2) }}</div>
        ...
        <div class="text-ink-gray-5 mt-1" style="font-size:8px">{{ s.label }}</div>
      </div>
    </div>
    <p class="text-p-xs mt-2 text-ink-gray-5">A bridge grant of {{ fPct(c.baseEq, 2) }} dilutes to {{ fPct(c.baseEq * c.retentionBase, 2) }} by the exit ({{ fPct(c.retentionBase, 0) }} retained). Tokens aren't diluted.</p>
  </div>
</template>
