...
const model = computed(() => {
  const c = props.c,
    ...
    sb = c.base,
    tgeFdv = sb.fdv;
  const oMap: Record<string, any> = Object.fromEntries(
    store.S.objectives.map((o: any) => [o.id, o]),
  );
  const valAt = (m: number) =>
    sb.netEqAt(c.baseEq * (1 + m), sb.exitVal) + c.baseTk * (1 + m) * tgeFdv;
  const rows: any[] = [];
  let cum = 0;
  rows.push({
    id: "base",
  ...
  if (c.capRaw > 0) {
    const earned = c.capEarned > 0;
    const prev = valAt(cum);
    cum += c.capRaw;
    rows.push({
      id: "capital",
      label: "Capital",
      kind: earned ? "earned" : "pending",
      from: prev,
      to: valAt(cum),
  ...
  const seen = new Set();
  (perf.achieved || []).forEach((id: string) => {
    const o = oMap[id];
    if (!o) return;
    seen.add(id);
    const reached = stageReached(plan, o.gate);
    const prev = valAt(cum);
    cum += o.uplift;
    rows.push({
      id,
...
const lo = (r: any) => Math.min(r.from, r.to);
const hi = (r: any) => Math.max(r.from, r.to);
</script>
...
<template>
  <div class="bg-surface-white rounded border border-outline-gray-1 p-5">
    ...
    <svg
      ...
      :style="{ height: H + 'px' }"
      role="img"
      :aria-label="`How the package grows: base-case net value from base through capital and gated objectives. Current ${fUSD(model.current)}, ceiling ${fUSD(model.ceiling)}.`"
    >
      <g v-for="(r, i) in model.rows" :key="r.id">
        <text
          ...
          text-anchor="end"
          class="fill-current text-ink-gray-8"
          font-size="11"
        >
          {{ r.label }}
        </text>
        <rect
          :x="x(lo(r))"
          :y="padT + i * rowH + 6"
          :width="Math.max(1, x(hi(r)) - x(lo(r)))"
          :height="rowH - 14"
          rx="2"
          :style="{
            fill: r.kind === 'available' ? 'none' : r.color,
            fillOpacity: r.kind === 'pending' ? 0.4 : r.kind === 'base' ? 1 : 0.92,
            stroke: r.kind === 'available' ? r.color : 'none',
            strokeDasharray: r.kind === 'available' ? '3 2' : '0',
          }"
        />
        <text
          :x="x(hi(r)) + 6"
