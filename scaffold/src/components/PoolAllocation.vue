...
const segments = computed(() => {
  const b = props.board;
  return [
    { label: `Equity (of ${fPct(b.esopNow, 0)} ESOP)`, used: b.sumEq, ceil: b.sumEqCeil, budget: b.esopNow },
    { label: `Tokens (board bucket ${fPct(b.boardBucket, 2)})`, used: b.sumTk, ceil: b.sumTkCeil, budget: b.boardBucket },
  ];
});
const w = (v: number, budget: number) => `${Math.max(0, Math.min(1.4, budget ? v / budget : 0)) * 100}%`;
  ...
  <div class="bg-surface-white rounded border border-outline-gray-1 p-5 space-y-4">
    ...
    <div v-for="s in segments" :key="s.label">
      ...
      <div class="relative h-2 rounded-full bg-surface-gray-2 overflow-hidden">
        <div class="absolute inset-y-0 left-0 rounded-full" :class="over(s) ? 'bg-surface-red-2' : 'bg-surface-gray-4'" :style="{ width: w(s.ceil, s.budget) }" />
