  ...
  <div class="rounded border border-outline-gray-1 overflow-hidden">
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-px bg-surface-gray-2">
      ...
      >
        ...
        <div class="h-[3px] mt-2 bg-surface-gray-2 rounded-full overflow-hidden">
          <div
            class="h-full rounded-full"
            :class="cell.accent ? 'bg-surface-amber-3' : 'bg-surface-gray-5'"
            :style="{ width: clamp(cell.v / max, 0, 1) * 100 + '%' }"
          />
        </div>
      </div>
    </div>
