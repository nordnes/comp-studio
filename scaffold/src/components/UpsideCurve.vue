...
const eqChart = computed(() => {
  ...
  for (let i = 0; i <= steps; i++) { const V = (topEq.value / steps) * i; labels.push(fUSD(V)); values.push(Math.round(Math.max(0, props.c.eqPct * (sb.value.retention * V - sb.value.strikeBasis)))); }
  return { labels, datasets: [{ name: 'net', values }] };
});
...
const tkChart = computed(() => {
  const steps = 12; const values: number[] = []; const labels: string[] = [];
  for (let i = 0; i <= steps; i++) { const F = (topFdv.value / steps) * i; labels.push(fUSD(F)); values.push(Math.round(props.c.tkPct * F)); }
  return { labels, datasets: [{ name: 'token value', values }] };
});
const areaOpts = { lineOptions: { regionFill: 1, hideDots: 1 }, tooltipOptions: { formatTooltipY: (v: number) => fUSD(v) } };
const lineOpts = { lineOptions: { hideDots: 1 }, tooltipOptions: { formatTooltipY: (v: number) => fUSD(v) } };
</script>
...
<template>
  <div class="bg-surface-white rounded border border-outline-gray-1 p-5">
    ...
    <div class="grid lg:grid-cols-2 gap-4">
      <div>
        <div class="text-xs text-ink-amber-3 mb-2">Equity · net vs exit company value</div>
      ...
      <div>
        <div class="text-xs text-ink-amber-3 mb-2">Tokens · value vs TGE FDV</div>
