...
const model = computed(() => {
  const c = props.c, sel = props.sel, p = store.S.plan, sbv = c.base;
...
const W = 560, H = 210, padL = 36, padR = 10, padT = 10, padB = 22;
...
function stackPath(key: 'tok' | 'base' | 'uplift') {
  ...
  const cum = (d: any) => key === 'tok' ? d.tok : key === 'base' ? d.tok + d.base : d.tok + d.base + d.uplift;
  const below = (d: any) => key === 'tok' ? 0 : key === 'base' ? d.tok : d.tok + d.base;
  let top = `M ${x(0)} ${y(cum(cd[0]))}`;
  for (let k = 1; k < cd.length; k++) top += ` L ${x(cd[k].m)} ${y(cum(cd[k - 1]))} L ${x(cd[k].m)} ${y(cum(cd[k]))}`;
  let bot = '';
  for (let k = cd.length - 1; k >= 1; k--) bot += ` L ${x(cd[k].m)} ${y(below(cd[k]))} L ${x(cd[k].m)} ${y(below(cd[k - 1]))}`;
  bot += ` L ${x(0)} ${y(below(cd[0]))} Z`;
  return top + bot;
}
const refLines = computed(() => {
  const out = [{ m: 12, label: 'cliff', color: '' }, { m: 24, label: 'Bad-Leaver', color: '#8C3A2B' }];
  if (model.value.tgeOffset > 0 && model.value.tgeOffset < 48) out.push({ m: model.value.tgeOffset, label: 'TGE', color: '#C46A1F' });
  if (model.value.nowOffset > 0 && model.value.nowOffset < 48) out.push({ m: model.value.nowOffset, label: 'today', color: '' });
  return out;
});
</script>
