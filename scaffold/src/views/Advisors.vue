...
<template>
  <div v-if="!sel || !c" class="text-center py-24 text-ink-gray-5">No advisor selected. Add one on the Board.</div>
    ...
    <div class="grid lg:grid-cols-12 gap-8">
      ...
      <div class="lg:col-span-5 space-y-6 no-print">
        <div class="bg-surface-white rounded border border-outline-gray-1 p-5 space-y-4">
          <div class="text-sm text-ink-gray-5">Profile</div>
          <div><div class="text-xs text-ink-gray-5 mb-1">Name</div><input :value="sel.name" class="w-full bg-transparent border-b border-outline-gray-2 text-sm text-ink-gray-9 outline-none py-1 focus:border-outline-gray-3" @input="e => setField('name', (e.target as HTMLInputElement).value)" /></div>
          <div><div class="text-xs text-ink-gray-5 mb-1">Sector</div><Select :model-value="sel.sector" :options="SECTORS.map(s => ({ label: s, value: s }))" @update:model-value="v => setField('sector', v)" /></div>
          <div class="grid grid-cols-2 gap-4">
            <div><div class="text-xs text-ink-gray-5 mb-1">Engagement (yrs)</div><NumIn :model-value="sel.years" :min="1" :max="10" aria-label="Years" @update:model-value="v => setField('years', v)" /></div>
            <div><div class="text-xs text-ink-gray-5 mb-1">Start date</div><input type="date" :value="sel.startDate || todayISO()" class="w-full bg-transparent border-b border-outline-gray-2 text-sm text-ink-gray-9 outline-none py-1" @input="e => setField('startDate', (e.target as HTMLInputElement).value)" /></div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div><div class="text-xs text-ink-gray-5 mb-1">Granted at</div><Select :model-value="sel.grantRound || 'bridge'" :options="roundList(S.plan).map(r => ({ label: roundLabel(S.plan, r), value: r }))" @update:model-value="v => setField('grantRound', v)" /></div>
            <div><div class="text-xs text-ink-gray-5 mb-1">Tax residency</div><Select :model-value="sel.taxResidency || 'Other'" :options="['UK', 'US', 'Other'].map(t => ({ label: t, value: t }))" @update:model-value="v => setField('taxResidency', v)" /></div>
          </div>
          <div><div class="text-xs text-ink-gray-5 mb-1">Notes</div><input :value="sel.notes" class="w-full bg-transparent border-b border-outline-gray-2 text-sm text-ink-gray-9 outline-none py-1" @input="e => setField('notes', (e.target as HTMLInputElement).value)" /></div>
        </div>

        <div class="bg-surface-white rounded border border-outline-gray-1 p-5 space-y-4">
          <div class="flex items-center justify-between">
            <div class="text-sm text-ink-gray-5">Base — denomination</div>
          ...
          <template v-if="sel.mode === 'tier'">
            ...
            <div class="grid grid-cols-3 gap-2">
              <button v-for="(t, ti) in S.tiers" :key="ti" class="p-3 text-left rounded border" :class="sel.tier === ti ? 'bg-surface-amber-2 border-outline-amber-2' : 'border-outline-gray-2 hover:bg-surface-gray-1'" @click="setField('tier', ti)">
                ...
                <div class="text-xs mt-1 tabular-nums text-ink-gray-5">{{ fPct(S.plan.baseGrant.equityPct * t.mult, 1) }} eq · {{ fPct(S.plan.baseGrant.tokenPct * t.mult, 1) }} tok</div>
              </button>
            </div>
            <EquityBenchmark :sel="sel" :c="c" />
          </template>
          <template v-else>
            <div><div class="text-xs text-ink-gray-5 mb-1">Annual value (USD)</div><NumIn :model-value="sel.annualValue" fmt="usd" :min="0" aria-label="Annual value" @update:model-value="v => setField('annualValue', v)" /></div>
          ...
          <div v-if="sel.hasCash"><div class="text-xs text-ink-gray-5 mb-1">Annual cash (USD)</div><NumIn :model-value="sel.cashAnnual" fmt="usd" :min="0" aria-label="Cash" @update:model-value="v => setField('cashAnnual', v)" /></div>
        </div>
        ...
        <div class="rounded border border-outline-amber-2 bg-surface-amber-2 p-5 space-y-4">
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div><div class="text-xs text-ink-gray-5 mb-1">Equity round</div><NumIn :model-value="sel.performance?.capitalEquity || 0" fmt="usd" :min="0" aria-label="Capital equity round" @update:model-value="v => setPerfField('capitalEquity', v)" /></div>
              <div><div class="text-xs text-ink-gray-5 mb-1">Token OTC (Foundation)</div><NumIn :model-value="sel.performance?.capitalToken || 0" fmt="usd" :min="0" aria-label="Capital token OTC" @update:model-value="v => setPerfField('capitalToken', v)" /></div>
            </div>
            <p class="text-p-xs mt-1 text-ink-gray-5">{{ fUSD(S.plan.capitalUplift.per) }} introduced → +{{ (S.plan.capitalUplift.pct * 100).toFixed(0) }}% of base, cap +{{ (S.plan.capitalUplift.cap * 100).toFixed(0) }}% · gate {{ ms[S.plan.capitalUplift.gate] }} · counts both channels</p>
          </div>
          <div class="space-y-2 pt-2 border-t border-outline-amber-2">
            <div class="text-xs text-ink-gray-5">Objectives · off / target / earned</div>
            <div v-for="o in S.objectives" :key="o.id" class="p-3 rounded border bg-surface-white"
              ...
              <div class="text-p-xs mt-1 text-ink-gray-5 leading-snug">{{ o.trigger }} · gate: {{ ms[o.gate] }}<span v-if="objState(o.id) === 'earned' && !stageReached(S.plan, o.gate)" class="text-ink-amber-3"> · ⏳ awaiting gate</span></div>
            ...
            <p class="text-p-xs pt-1 text-ink-gray-5">Earned: <span class="text-ink-green-3">+{{ (c.earnedUplift * 100).toFixed(0) }}%</span><span v-if="c.pendingUplift > 0"> · pending gate: <span class="text-ink-amber-3">+{{ (c.pendingUplift * 100).toFixed(0) }}%</span></span> · ceiling +{{ (c.ceilUplift * 100).toFixed(0) }}%</p>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-7 space-y-6 print-area">
        ...
        <template v-if="showDetail">
          ...
          <div class="bg-surface-white rounded border border-outline-gray-1 p-5">
            <div class="text-sm text-ink-gray-5 mb-3">Scenario range · net value (low → high)</div>
          ...
          <div class="grid sm:grid-cols-2 gap-6">
            <DilutionPath :c="c" />
            <div class="bg-surface-white rounded border border-outline-gray-1 p-5">
              <div class="text-sm text-ink-gray-5 mb-3">Instruments · net of strike</div>
              ...
              <p class="text-p-xs mt-2 text-ink-gray-5">Strike subject to an HMRC SAV / 409A valuation agreed before first grant.</p>
            </div>
          </div>
        </template>
