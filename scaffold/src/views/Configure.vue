  ...
  <div data-theme="dark" class="-mx-3 sm:-mx-5 -my-6 sm:-my-8 min-h-[calc(100vh-3rem)] bg-surface-white text-ink-gray-9">
    <div class="mx-auto w-full max-w-7xl px-3 sm:px-5 py-8 space-y-6">
      ...
      <div class="rounded border border-outline-gray-2 bg-surface-gray-2 p-3 flex items-center gap-2 flex-wrap">
        ...
        <span class="text-p-xs text-ink-gray-6">columns: scope,round,postMoney,raise,esopPct,tgeMult — ingest cap-table / dilution-model updates</span>
      </div>
      ...
      <div class="rounded border border-outline-gray-2 bg-surface-gray-2 p-4">
        ...
        <div class="grid sm:grid-cols-4 gap-4">
          ...
          <div><div class="text-xs text-ink-gray-6 mb-1">ESOP at adoption</div><Select :model-value="S.S.plan.esopStart" :options="esopOpts" @update:model-value="v => setP('esopStart', Number(v))" /></div>
          <div><div class="text-xs text-ink-gray-6 mb-1">ESOP cap</div><NumIn :model-value="S.S.plan.esopCap" fmt="pct" aria-label="ESOP cap" @update:model-value="v => setP('esopCap', v)" /></div>
        </div>
      </div>
          ...
          <div v-for="sk in Object.keys(S.S.plan.scenarios)" :key="sk" class="rounded border p-3"
            ...
            <div class="grid sm:grid-cols-4 gap-3">
              <div v-for="rd in S.S.plan.rounds" :key="rd.id" class="flex gap-2">
                <div class="flex-1"><div class="text-xs text-ink-gray-6 mb-1">{{ rd.label }} post</div><NumIn :model-value="S.S.plan.scenarios[sk][rd.id]?.post" fmt="usd" :aria-label="`${rd.label} post`" @update:model-value="v => setPath(['plan','scenarios', sk, rd.id, 'post'], v)" /></div>
                <div class="w-14"><div class="text-xs text-ink-gray-6 mb-1">ESOP</div><NumIn :model-value="S.S.plan.scenarios[sk][rd.id]?.esop" fmt="pct" aria-label="ESOP" @update:model-value="v => setPath(['plan','scenarios', sk, rd.id, 'esop'], v)" /></div>
              </div>
              <div><div class="text-xs text-ink-gray-6 mb-1">TGE ×</div><NumIn :model-value="S.S.plan.scenarios[sk].tgeMult" fmt="mult" aria-label="TGE multiple" @update:model-value="v => setPath(['plan','scenarios', sk, 'tgeMult'], v)" /></div>
            </div>
          </div>
        </div>
        <div class="grid sm:grid-cols-3 gap-4 mt-3">
          <div><div class="text-xs text-ink-gray-6 mb-1">TGE FDV anchor (round before TGE)</div><Select :model-value="S.S.plan.tgeAnchor" :options="anchorOpts()" @update:model-value="v => setP('tgeAnchor', v)" /></div>
      ...
      <div class="rounded border border-outline-gray-2 bg-surface-gray-2 p-4">
        ...
        <div class="grid sm:grid-cols-4 gap-4">
          ...
          <div><div class="text-xs text-ink-gray-6 mb-1">Advisor token pool</div><NumIn :model-value="S.S.plan.advisorTokenPoolPct" fmt="pct" aria-label="Advisor token pool" @update:model-value="v => setP('advisorTokenPoolPct', v)" /></div>
          <div><div class="text-xs text-ink-gray-6 mb-1">…committed (ecosystem)</div><NumIn :model-value="S.S.plan.committedAdvisorTokenPct" fmt="pct" aria-label="Committed" @update:model-value="v => setP('committedAdvisorTokenPct', v)" /></div>
          <div><div class="text-xs text-ink-gray-6 mb-1">Board bucket (ring-fenced)</div><NumIn :model-value="S.S.plan.boardTokenBucketPct" fmt="pct" aria-label="Board bucket" @update:model-value="v => setP('boardTokenBucketPct', v)" /></div>
          ...
          <div><div class="text-xs text-ink-gray-6 mb-1">TGE date</div><input type="date" :value="S.S.plan.tgeDate" class="w-full bg-surface-white border border-outline-gray-3 rounded px-2 py-1 text-sm text-ink-gray-9 outline-none" @input="e => setP('tgeDate', (e.target as HTMLInputElement).value)" /></div>
        </div>
      </div>
      ...
      <div class="rounded border border-outline-gray-2 bg-surface-gray-2 p-4">
        ...
        <div class="grid sm:grid-cols-4 gap-4">
          ...
          <div><div class="text-xs text-ink-gray-6 mb-1">Cap %base</div><NumIn :model-value="S.S.plan.capitalUplift.cap" fmt="pct" aria-label="Cap" @update:model-value="v => setPath(['plan','capitalUplift','cap'], v)" /></div>
          <div><div class="text-xs text-ink-gray-6 mb-1">Gate</div><Select :model-value="S.S.plan.capitalUplift.gate" :options="msOpts()" @update:model-value="v => setPath(['plan','capitalUplift','gate'], v)" /></div>
        </div>
      </div>
        ...
        <div class="grid sm:grid-cols-3 gap-4">
          <div v-for="(t, i) in S.S.tiers" :key="i" class="rounded border border-outline-gray-2 bg-surface-gray-2 p-4">
            ...
            <div class="grid grid-cols-2 gap-2">
              ...
              <div><div class="text-xs text-ink-gray-6 mb-1">= Equity</div><div class="font-display text-lg tabular-nums text-ink-gray-9">{{ fPct(S.S.plan.baseGrant.equityPct * (t.mult || 1), 2) }}</div></div>
            </div>
          </div>
        </div>
      </div>
