...
const kpis = computed(() => [
  ...
  { l: `Range ${S.value.plan.scenarios[sk.value[0]].label}→${S.value.plan.scenarios[sk.value[sk.value.length - 1]].label}`, v: `${fUSD(board.value.cost[sk.value[0]] || 0)} – ${fUSD(board.value.cost[sk.value[sk.value.length - 1]] || 0)}`, accent: false, sub: '' },
...
const flags = computed(() => {
  ...
  out.push({ t: 'note', m: 'TGE multipliers (2×/5×/12×) unvalidated — confirm before sharing externally.' });
...
<template>
  ...
  <div v-else class="space-y-8">
    <PageHeader title="The advisory board, at a glance."
      ...
      <template #actions><Button variant="solid" theme="gray" icon-left="lucide-sliders-horizontal" label="Configure" route="/configure" /></template>
    </PageHeader>
