...
const shown = computed(() => {
  ...
  return S.value.objectives.filter(o => perf.achieved?.includes(o.id) || perf.targeted?.includes(o.id));
});

function propText(): string {
  const a = sel.value!, cc = c.value!, b = sb.value!, plan = S.value.plan;
  return [
    `RAIKU LABS — ADVISORY ENGAGEMENT PROPOSITION`,
    `Confidential discussion draft · ${a.name}`, a.sector, '',
    ...
    ...shown.value.map(o => `  ${o.label}: +${(o.uplift * 100).toFixed(0)}% — ${o.trigger} (gate: ${ms.value[o.gate]}).`),
...
async function copyProp() { try { await navigator.clipboard.writeText(propText()); flash('Copied'); } catch { flash('Clipboard blocked'); } }
function print() { window.print(); }

const residencyLine = computed(() => sel.value?.taxResidency === 'UK'
  ...
  <div v-else class="space-y-8">
    <div class="flex justify-between items-center flex-wrap gap-3 no-print">
      ...
      <div class="flex items-center gap-2">
        ...
        <Button variant="subtle" theme="gray" icon-left="lucide-copy" label="Copy" @click="copyProp" />
      </div>
    </div>
